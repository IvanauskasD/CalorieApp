...
import { getNotApprovedFoods, approveFood } from '../../actions/food'
...
const FoodSuggestionTable = ({
  ...
  const test = {
    ...
    useEffect(() => {
      ...
      // if(dietprofile !== null && dietprofile.dietprofile !== null) {
      //   setForm(dietprofile.dietprofile.user._id)
      // }
        getNotApprovedFoods();
        if (food !== null) {
            setLoaded(true)
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={row.name}>
              ...
              <TableCell style={{ width: 160 }} align="right">
                {/* {(row.votedOnBy).map((ro) => (
                  ro === localStorage.getItem('dietprofile') ? (
                    <button ref={btnRef} onClick={() => {onSubmit(row._id)}} className='btn btn-success'>Reset Vote</button>
                  ) : (
...
export default connect(mapStateToProps, {getCurrentDietProfile, approveFood, getNotApprovedFoods })(FoodSuggestionTable);